...
public class CustomerController {
	...
	@PostMapping("/getid")
	public int  get_id(@RequestBody Customer customer) throws InvalidUserOrPasswordException {
		int  response;
		try {
			response = ls.getId(customer);
	...
	public String deleteCustomer(@PathVariable int customerId) throws CustomerNotFoundException {
		String s = null;
		try {
			s = cusService.deleteCustomer(customerId);

		} catch (Exception e) {
			throw new CustomerNotFoundException("Customer with given ID: " + customerId+ " Not Found to Delete");
		}
		return s;
